...
function renderChildren(children) {
  ...
  Array.prototype.forEach.call(children, (node) => {
    if(NodeTool.getNodeName(node) === 'mo'){
            ...
            if(Brackets.isPair(nearLeft, op)){
              ...
              if(Brackets.isLeft(op)) {
                ...
                console.error("bracket not match");
                  parts.push(Brackets.parseRight(op, false));
              }
            }
          }else {
            // some brackets left side is same as right side.
      ...
      if(lefts[lefts.length - 1] == "|" || lefts[lefts.length - 1] == "||") lefts.pop();
  });
  // 這裏非常不嚴謹
  if(lefts.length > 0){
...
function getRender(node) {
  ...
  switch(nodeName){
    ...
    case 'msup':
      ...
      break;
    case 'msubsup':
      render = getRender_default("{@1}_{@2}^{@3}");
      break;
    case 'mover':
      render = renderMover;
      break;
    case 'munder':
      ...
      break;
    case 'munderover':
      render = getRender_default("\\overset{@3}{\\underset{@2}{@1}}");//"@1\\limits_{@2}^{@3}"
      break;
    case 'mmultiscripts':
...
function renderTable(node, children) {
  const template = "\\begin{aligned} @content \\end{aligned}";
...
function renderMover(node, children){
  ...
  for(let i = 0; i < nodes.length - 1; i++) {
    ...
    const template = getMatchValueByChar({
      ...
      judgeChar: over,
      defaultValue: "\\overset{@2}{@1}"//"@1\\limits^{@2}"
    });
...
function renderMunder(node, children){
  ...
  for(let i = 0; i < nodes.length - 1; i++) {
    ...
    const template = getMatchValueByChar({
      ...
      judgeChar: under,
      defaultValue: "\\underset{@2}{@1}" //"@1\\limits_{@2}"
    });
